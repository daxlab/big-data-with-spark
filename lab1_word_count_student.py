...
wordCounts = wordPairs.reduceByKey(lambda a,b: b+a)
...
wordCountsCollected = (wordsRDD
                       .map(lambda x: (x,1)).reduceByKey(lambda a,b: b+a)
                       .collect())
...
average = totalCount / float(len(wordCounts.map(lambda (x,y):x).distinct().collect()))
print totalCount
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda x: (x,1)).reduceByKey(lambda a,b: b+a)
